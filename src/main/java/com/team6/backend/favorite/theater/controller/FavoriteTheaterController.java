...
public class FavoriteTheaterController {
    ...
    public ResponseEntity<List<FavoriteTheaterResponseDto>> getFavoriteTheaterList(HttpServletRequest httpServletRequest) {
        ...
        String email = jwtUtil.getEmailFromToken(accessToken);
        List<FavoriteTheaterResponseDto> favoriteTheaterList = favoriteTheaterService.getAllFavoriteTheatersByEmail(email);
        return ResponseEntity.ok(favoriteTheaterList);
    }
}
